...
def test_shadow_regime_settings_parse_and_validate(monkeypatch) -> None:
    ...
    monkeypatch.setenv("SHADOW_FOLLOW_MAX_PRICE", "0.80")
    ...
    assert settings.shadow_follow_max_price == 0.80
    ...
    with pytest.raises(ValueError, match="SHADOW_FOLLOW_MIN_PRICE"):
        Settings(shadow_follow_min_price=0.80, shadow_follow_max_price=0.70)
    monkeypatch.delenv("SHADOW_ENABLE_INVERT_BRANCH")
    assert Settings(_env_file=None).shadow_enable_invert_branch is True
