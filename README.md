...
- `SHADOW_FOLLOW_MAX_PRICE`: optional exclusive upper bound for `price_filter` follow trades. When set with `SHADOW_FOLLOW_MIN_PRICE`, follow only when the shadow Up executable price is in `[min, max)`.
